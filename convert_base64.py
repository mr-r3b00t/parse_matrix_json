...
def parse_message_block(lines):
    ...
    if content_lines:
        ...
        message_dict["message"] = {
            "content": [
                {"login": item["login"], "password": encode_base64(item["password"])}
                for item in message_content
            ]
        }
    elif "message" in message_dict:
        message_dict["message"] = encode_base64(message_dict["message"])
...
def refactor_file(input_file, output_file):
    """Refactor the input file into structured JSON with Base64-encoded messages."""
...
print(f"Refactored JSON with Base64 encoding written to {output_file}")
